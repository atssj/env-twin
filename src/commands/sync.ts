...
export async function runSync(
      ...
      if (bulkDecision === 'all_copy') {
        ...
        const action = await select<string>(
          ...
          [
            ...
            sourceValue
              ? { title: `Copy from ${sourceDescription} (${key}=${sourceValue})`, value: 'copy' }
              : null,
  ...
  // If it DOES exist, we handle orphans (promotion)
    ...
    for (const fileName of orphanFiles) {
      ...
      if (orphans.length > 0) {
        const potentialPromotions = orphans.filter(
          k =>
            !report.allKeys.has(k) ||
            !report.files.find(f => f.fileName === sourceOfTruth)?.keys.has(k)
        );

        if (potentialPromotions.length > 0 && !options.yes) {
          console.log(
            colors.yellow(
              `Found ${potentialPromotions.length} keys in ${fileName} that are missing in ${sourceOfTruth}:`
            )
          );
  ...
  for (const fileName of filesToUpdate) {
    ...
    try {
      // Check for existing file stats (permissions)
      ...
      try {
        ...
        // Ignore error if file doesn't exist or can't be stat-ed
      }

      fs.writeFileSync(tempPath, newContent, 'utf-8');

      // Restore permissions or set secure defaults
      if (originalMode) {
        try {
        ...
        // New file: if it's an environment file (but not example), default to 600
